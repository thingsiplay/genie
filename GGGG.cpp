...
char *BCX_TmpStr(size_t Bites, size_t iPad, int iAlloc) {
  ...
  if (Bites * sizeof(char) > BCX_MAX_VAR_SIZE) {
    printf("Buffer Overflow caught in BCX_TmpStr - requested space of %d "
           "EXCEEDS %d\n",
...
#endif
  if (iAlloc)
    StrFunc[StrCnt] = (char *)calloc(Bites + iPad + 1, sizeof(char));
...
std::string left(std::string s, int length) { return s.substr(0, length); }
char *left(const char *S, int length) {
  register int tmplen = strlen(S);
  if (length < 1)
    return BCX_TmpStr(1, 0, 1);
  if (length < tmplen)
    tmplen = length;
...
char *trim(const char *S) {
  ...
  while (i > 0 && (S[i - 1] == 32 || (S[i - 1] >= 9 && S[i - 1] <= 13)))
    i--;
...
char *replace(const char *src, const char *pat, const char *rep) {
  ...
  repsz = strlen(rep);
  patsz = strlen(pat);
  for (tmpsz = 0, p = (char *)src; (q = _strstr_(p, (char *)pat)) != 0;
       p = q + patsz)
  ...
  if (!strtmp)
    return NULL;
  for (r = strtmp, p = (char *)src; (q = _strstr_(p, (char *)pat)) != 0;
       p = q + patsz) {
    delta = (size_t)(q - p);
    memcpy(r, p, delta);
    r += delta;
...
int instr(const char *mane, const char *match, int offset, int sensflag) {
  ...
  if (!mane || !match || !*match || offset > (int)strlen(mane))
    return 0;
  if (sensflag)
    s = _stristr_(offset > 0 ? (char *)mane + offset - 1 : (char *)mane,
                  (char *)match);
  else
    s = _strstr_(offset > 0 ? (char *)mane + offset - 1 : (char *)mane,
                 (char *)match);
...
char *_stristr_(char *String, char *Pattern) {
  ...
  while (Pattern[++mi]) {
    if (String[mi] == 0)
      return 0;
    if (LowCase[(unsigned char)String[mi]] !=
        LowCase[(unsigned char)Pattern[mi]]) {
      String++;
      mi = -1;
    }
  }
...
int Split(char Buf[][cSizeOfDefaultString], const char *T, const char *Delim,
          int Flg) {
  int Begin = 0;
  int Count = 0;
  int Quote = 0;
  ...
  if ((Flg & 1) == 0) // 0 if false
    for (i = 0; i < Count; i++)
      strcpy(Buf[i], (char *)RemoveStr(Buf[i], Chr34));
  return Count;
}
...
int main(int argc, char *argv[]) {
  ...
  for (Lnum = 0; Lnum <= Codes; Lnum += 1) {
    ...
    if (Line[Lnum][0] != 0) {
      ...
      *Dec = 0;
      if (instr(Code, ":")) {
        if (str_cmp(Bit, "pce") == 0 ||
            (str_cmp(Bit, "sms") == 0 && lof(File1) % 1024)) {
          Off += 512;
        }
      ...
      if (str_cmp(Type, "1") == 0) {
        if (strlen(Code) == 6) {
          strcpy(Code, join(4, hex(Hex2Dec(mid(Code, 6, 1)) ^ 15),
                            mid(Code, 3, 3), ":", left(Code, 2)));
          ...
          for (Num = 0; Num <= lof(File1) / 8192; Num += 1) {
            if (Off < lof(File1)) {
              fseek(FP2, Off, 0);
              PUT(FP2, chr(Rep), 1);
              strcpy(Out, join(9, Line[Lnum], " - ", trim(hex(Off)), ":",
                               trim(hex(Cmp)), ":", trim(hex(Rep)), CRLF, Out));
            }
            Off += 8192;
          }
        }
        if (strlen(Code) == 9) {
          strcpy(Code, join(7, hex(Hex2Dec(mid(Code, 6, 1)) ^ 15),
                            mid(Code, 3, 3), ":", left(Code, 2), ":",
                            mid(Code, 7, 1), mid(Code, 9, 1)));
          strcpy(Dec, lpad(Bin(Hex2Dec(mid(Code, 9, 2))), 8, 48));
          strcpy(Code, join(2, left(Code, 8),
                            hex(Bin2Dec(join(2, mid(Dec, 7, 2), left(Dec, 6))) ^
                                186)));
          Off = Hex2Dec(left(Code, 4));
      ...
      if (str_cmp(Type, "2") == 0) {
        ...
        strcpy(Code, join(6, mid(Dec, 17, 8), mid(Dec, 9, 8), right(Dec, 8),
                          mid(Dec, 30, 3), mid(Dec, 25, 5), left(Dec, 8)));
        strcpy(Dec, join(3, lpad(hex(Bin2Dec(left(Code, 24))), 6, 48), ":",
                         lpad(hex(Bin2Dec(right(Code, 16))), 4, 48)));
        ...
        if (Off < lof(File1)) {
          ...
          fseek(FP2, Off + 1, 0);
          PUT(FP2, chr(Rep), 1);
          strcpy(Out, join(8, Line[Lnum], " - ", trim(hex(Off)), ":",
                           trim(hex(Cmp)), trim(hex(Rep)), CRLF, Out));
        }
      }
      if (str_cmp(Type, "3") == 0) {
        ...
        if (strlen(Code) == 6) {
          strcpy(Code, join(10, mid(Dec, 9, 1), mid(Dec, 14, 4),
                            mid(Dec, 22, 3), mid(Dec, 5, 1), mid(Dec, 10, 4),
                            mid(Dec, 18, 3), mid(Dec, 1, 1), mid(Dec, 6, 3),
                            mid(Dec, 21, 1), mid(Dec, 2, 3)));
          strcpy(Dec, join(3, lpad(hex(Bin2Dec(left(Code, 16))), 4, 48), ":",
        ...
        if (strlen(Code) == 8) {
          strcpy(Code, join(14, mid(Dec, 9, 1), mid(Dec, 14, 4),
                            mid(Dec, 22, 3), mid(Dec, 5, 1), mid(Dec, 10, 4),
                            mid(Dec, 18, 3), mid(Dec, 1, 1), mid(Dec, 6, 3),
                            mid(Dec, 29, 1), mid(Dec, 2, 3), mid(Dec, 25, 1),
                            mid(Dec, 30, 3), mid(Dec, 21, 1), mid(Dec, 26, 3)));
          strcpy(Dec, join(5, lpad(hex(Bin2Dec(left(Code, 16))), 4, 48), ":",
                           lpad(hex(Bin2Dec(mid(Code, 17, 8))), 2, 48), ":",
                           lpad(hex(Bin2Dec(right(Code, 8))), 2, 48)));
      ...
      if (str_cmp(Type, "4") == 0) {
        ...
        strcpy(Code, join(3, lpad(Bin(Hex2Dec(mid(Dec, 3, 2))), 8, 48),
                          lpad(Bin(Hex2Dec(mid(Dec, 5, 2))), 8, 48),
                          lpad(Bin(Hex2Dec(mid(Dec, 7, 2))), 8, 48)));
        strcpy(Dec, join(7, mid(Code, 11, 4), mid(Code, 19, 4), left(Code, 4),
                         mid(Code, 23, 2), mid(Code, 9, 2), mid(Code, 5, 4),
                         mid(Code, 15, 4)));
        strcpy(Code,
               join(5, lpad(hex(Bin2Dec(left(Dec, 8))), 2, 48),
                    lpad(hex(Bin2Dec(mid(Dec, 9, 8))), 2, 48),
        ...
        if ((UCHAR) * (Bit) != 33 && (UCHAR) * (Bit) != 49) {
          Off = Bin2Dec(join(3, "0", left(lpad(Bin(Hex2Dec(Dec)), 24, 48), 8),
        ...
        if (Off < lof(File1)) {
          fseek(FP2, Off, 0);
          PUT(FP2, chr(Rep), 1);
          strcpy(Out, join(7, Line[Lnum], " - ", trim(hex(Off)), ":",
                           trim(hex(Rep)), CRLF, Out));
        }
      }
    }
  }
